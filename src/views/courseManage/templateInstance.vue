<template>
  <div>
    <el-dialog title="复制模板" :visible.sync="copyDialogVisible" width="30%">
      <el-form label-position="top">
        ...
        <br />
        <div>
          <el-popconfirm title="确定要复制吗？" @confirm="handleCopy">
    ...
    <el-card>
      ...
      <el-table border :data="instanceList" style="width: 100%">
        ...
        <el-table-column label="成绩册">
          <template slot-scope="scope">
            <div
              v-if="scope.row.excel"
              class="excel-download"
              @click="handleDownloadExcel(scope.row.excel.id, scope.row.excel.name)"
            >
              {{ scope.row.excel.name }}
            </div>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" @click="handleEdit(scope.row.name, scope.row.id)">编辑 </el-button>
            <el-button size="mini" @click="openCopyDialog(scope.row.course.courseId, scope.row.id)">复制 </el-button>
            <el-button size="mini" @click="handleDownload(scope.row.name, scope.row.id)">生成文档 </el-button>
            <el-button size="mini" type="danger" @click="handleDelete(scope.row.name, scope.row.id)">删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
