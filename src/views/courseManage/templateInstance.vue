      ...
      <el-form label-position="top">
        ...
        <el-form-item label="请选择复制到哪个课程：">
          <el-select v-model="targetCopyCourseId" placeholder="请选择">
            <el-option
              v-for="item in courses.filter(
                ({ courseId }) => courseId !== selectCopyCourseId
              )"
              :key="item.courseId"
        ...
        <br />
        <div>
          <el-popconfirm title="确定要复制吗？" @onConfirm="handleCopy">
            <el-button slot="reference">复制</el-button>
          </el-popconfirm>

          <el-button @click="() => (copyDialogVisible = false)" type="primary"
            >取消</el-button
          >
        </div>
      </el-form>
    ...
    <el-card>
      <template slot="header">
        ...
        <el-button
          ...
          size="mini"
          type="warning"
          @click="handleNew($route.query.templateId)"
          >创建新实例</el-button
        >
      </template>
        ...
        <el-table-column label="成绩册">
          <template slot-scope="scope">
            <div
              ...
              @click="
                handleDownloadExcel(scope.row.excel.id, scope.row.excel.name)
              "
            >
...
export default {
  ...
  methods: {
    ...
    async handleDownloadExcel(excelId, excelName) {
      ...
      fileDownload(
        res.data,
        `${excelName.substr(0, excelName.lastIndexOf("."))}.docx`
      );
    },
    ...
    async openCopyDialog(courseId, instanceId) {
      ...
      this.$notify({
        ...
        type: "warning",
        message:
          "只能复制到没有绑定教学文档实例的课程，若已绑请先查看并删除对应课程的文档实例",
        duration: 10000,
